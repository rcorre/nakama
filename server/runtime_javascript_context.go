...
func NewRuntimeJsContext(r *goja.Runtime, node string, env goja.Value, mode RuntimeExecutionMode, queryParams map[string][]string, sessionExpiry int64, userID, username string, vars map[string]string, sessionID, clientIP, clientPort string) goja.Value {
	ctxObj := r.NewObject()
...
func NewRuntimeJsInitContext(r *goja.Runtime, node string, env map[string]string) goja.Value {
	ctxObj := r.NewObject()
	ctxObj.Set(__RUNTIME_JAVASCRIPT_CTX_NODE, node)
	ctxObj.Set(__RUNTIME_JAVASCRIPT_CTX_ENV, env)
